...
const particles = new THREE.Points(pg, new THREE.PointsMaterial({
  size: 0.03, color: 0x7af6ff, transparent: true, opacity: 0.5, blending: THREE.AdditiveBlending, depthWrite: false
}));
scene.add(particles);

// ---------------------------------------------------------------------------
// Lighting — restrained, with a warm rim for the "premium" read
// ---------------------------------------------------------------------------
scene.add(new THREE.AmbientLight(0x1a2336, 0.7));
const key = new THREE.DirectionalLight(0xffffff, 1.1); key.position.set(-3, 4, 3); scene.add(key);
const rim = new THREE.PointLight(0x7af6ff, 2.0, 22); rim.position.set(0, 0, -2); scene.add(rim);
const warm = new THREE.PointLight(0xffcaa0, 0.9, 26); warm.position.set(4, -1.5, 1.5); scene.add(warm);
